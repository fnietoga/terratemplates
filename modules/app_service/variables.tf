...
variable "app_php_version" {
  ...
  validation {
    condition     = contains(["5.5","5.6","7.0","7.1","7.2","7.3","7.4",null], var.app_php_version)
...
variable "app_python_version" {
  ...
  validation {
    condition     = contains(["2.7", "3",null], var.app_python_version)
...
locals {
  app_fw_ips = concat(var.app_allowed_ips, "${chomp(data.http.myip.body) != "" ? chomp(data.http.myip.body) : null}")
